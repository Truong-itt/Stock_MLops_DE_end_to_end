...
thu thập giá cổ phiếu trực tiếp từ Yahoo Finance WebSocket, xử lý qua pipeline Kafka → Flink → ClickHouse / ScyllaDB, phân tích sentiment tin tức bằng mô hình FinBERT, và hiển thị trực quan qua dashboard web thời gian thực.
...
└────────────┬───────────────────┘
             ...
             ▼
┌────────────────────────────────┐   ┌────────────────────────────────┐
│   ClickHouse  (OLAP Warehouse) │──▶│   agg_worker.py               │
│   Materialized Views:          │   │   Đọc OHLCV từ ClickHouse      │
│   1m / 5m / 1h / 3h / 6h OHLCV│   │   → Write ScyllaDB (60s/lần)  │
└────────────────────────────────┘   └──────────────────┬─────────────┘
                                                         │
                                                         ▼
                                     ┌────────────────────────────────┐
...
│
├── clickhouse_service/          # ClickHouse OLAP Warehouse
│   ├── docker-compose.yml
│   └── init/
│       └── 01_create_tables.sql # Raw ticks + Materialized Views (1m/5m/1h/3h/6h)
│
...
| **OLAP Warehouse** | ClickHouse | 24.3 (`clickhouse-server:24.3-alpine`) |
...
### ClickHouse — Database `stock_warehouse`
...
- **ClickHouse** sử dụng `AggregatingMergeTree` + Materialized Views để tính OHLCV real-time.
